...
for iX = whichSessions
    ...
    pupilEarly(iX,:) = smooth(mean(eyeData(iX).eta.alignedFace{1}(earlyTrials,:,1),1));
    pupilLate(iX,:) = smooth(mean(eyeData(iX).eta.alignedFace{1}(lateTrials,:,1),1));
    whiskEarly(iX,:) = smooth(mean(eyeData(iX).eta.alignedFace{1}(earlyTrials,:,2),1));
    whiskLate(iX,:) = smooth(mean(eyeData(iX).eta.alignedFace{1}(lateTrials,:,2),1));
    pawEarly(iX,:) = smooth(mean(eyeData(iX).eta.alignedFace{1}(earlyTrials,:,4),1));
    pawLate(iX,:) = smooth(mean(eyeData(iX).eta.alignedFace{1}(lateTrials,:,4),1));
end
