...
%plot all sessions for 2 animals 
...
%% Or just one single session (1 animal) 
mouseList = {{'LEW031'}};
expList = {{'2020-02-03',1,[1]}};
...
[~, whichTrials] = selectCondition(expInfo, getUniqueContrasts(expInfo), behavioralData,...
    initTrialConditions('movementTime','late','preStimMovement','quiescent','specificRTs',[.1 3]));
...
%%
k=1;
%plot raster of neural activity & facemap ROIs 
%choose to have trials sorted by pre-stim whisking (200-0 ms)or 1st move
figure;
...
figure; 
rasterBrowser_earlyLate(expInfo, behavioralData, neuralData, 'all', k)
figure;
rasterBrowser_earlyLate_Facemap(expInfo, behavioralData, eyeData, 'all', k)
