...
if strcmp(movementTime,'>0.5')
    ...
    %by reward amount
    [am{1,1}, am{2,1}, am{3,1}] = getHighLoRewardTrials(expInfo, behavioralData);
else
    %by stimulus contrast
    for i = 1:length(contrasts)
...
%split trials as REWARD x DIRECTION (nRews x nDirs)    
for a = 1:length(am)
    am_di{a,1} = intersect(am{a},di{1});
...
%split trials as CONTRAST x DIRECTION x OUTCOME (nContrasts x nDirs x nOutcomes)    
for d = 1:length(dirs)
    for h = 1:length(outcomes)
        for c = 1:length(contrasts)
            if strcmp(movementTime,'>0.5')
                [~, co_di_ou{c,d,h}] = selectCondition(expInfo, contrasts(c), behavioralData, initTrialConditions('repeatType', 'random', 'movementTime','all','specificRTs',[.5 Inf],'movementDir',dirs{d},'responseType',outcomes{h}));
            else
                [~, co_di_ou{c,d,h}] = selectCondition(expInfo, contrasts(c), behavioralData, initTrialConditions('repeatType', 'random', 'movementTime',movementTime,'movementDir',dirs{d},'responseType',outcomes{h}));
            end
        end
    end
end
for d = 1:size(co_di_ou,2)
    for h = 1:size(co_di_ou,3)
        si_di_ou{1,d,h} = cat(2, co_di_ou{contrasts<0,d,h});
        si_di_ou{2,d,h} = cat(2, co_di_ou{contrasts==0,d,h});
        si_di_ou{3,d,h} = cat(2, co_di_ou{contrasts>0,d,h});
    end
end
...
trialTypes.intVar.all = struct(...
    ...
    'contrast_direction_outcome',{co_di_ou},...
    'side_direction',{si_di},...
    'side_outcome',{si_ou},...
    'side_block',{si_bl},...
    'side_direction_block',{si_di_bl},...
    'side_direction_outcome',{si_di_ou},...
