function plotRTs(expInfo, behavioralData, blockTag)
    ...
    for ex = 1:size(expInfo,2)
        ...
        for t = 1:tLen   
            ...
            for iCond = 1:length(conds)
                ...
                responseTimes(responseTimes > 3) = NaN;
                meanRTs(t,iCond,ex) = mean(responseTimes);
                semRTs(t,iCond,ex) = std(responseTimes)/sqrt(length(responseTimes));
            end
    ...
    for t = 1:tLen
        ...
        errorbar(contrasts,nanmean(meanRTs(t,:,:),3),nanstd(meanRTs(t,:,:),[],3)/sqrt(size(meanRTs,3)),'o','MarkerFaceColor',color,'LineStyle','none','color',color,'MarkerEdgeColor','none','capsize',0);
    ...
    line([0 0],[0 2],'linestyle','--','color',[.5 .5 .5])
    axis([-1.2 1.2 0. 1.2])
