function theAxis = subplot(varargin)
    ...
    inset = [.1, .09, .02, .05]; % [left bottom right top]
    ...
    if narg == 1
        ...
        elseif isgraphics(arg) || isa(arg,'matlab.graphics.Graphics')
            ...
            elseif isa(h, 'matlab.graphics.chart.Chart') && ~isappdata(h,'SubplotPosition')
    ...
    elseif moveAxis && ~preventMove
        ...
        units = h.Units;
            ...
            %sealed charts position by outer position only & don't support
...
function out = isSubplotCandidate(obj)
    out = isgraphics(obj,'axes') || isgraphics(obj,'polaraxes') || ... 
