function SVCA(neuralData,npc)

%% subtract each neuron's mean FR from its trace
...
trim = size(rm_ms,2) - mod(size(rm_ms,2),2);

F_train = rm_ms(1:2:end,1:2:trim);
F_test = rm_ms(1:2:end,2:2:trim);

G_train = rm_ms(2:2:end,1:2:trim);
G_test = rm_ms(2:2:end,2:2:trim);

%%
npc = min([size(F_test,1) size(G_test,1)]);
cov = F_train * G_train';
[u,~,v] = svd(cov,'econ');
u = u(:,1:npc);
v = v(:,1:npc);
