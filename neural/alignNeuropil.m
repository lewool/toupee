function [neuralData] = alignNeuropil(expInfo, neuralData, behavioralData, timeBeforeAndAfter, events)
...
%             alignedResps{ev} = zeros(nt, length(eventWindow), size(neuralData(ex).cellResps,2));
            alignedNeuropil{ev} = zeros(nt, length(eventWindow), size(neuralData(ex).allFcell,2));
...
%                 alignedNeuropil{ev}(:,:,iPlane) = interp1(neuralData(ex).respTimes,neuralData(ex).cellResps(:,iCell),periEventTimes,'previous');
%             end
            
            for iPlane = 1:size(neuralData(ex).allFcell,2)
                try
                    alignedNeuropil{ev}(:,:,iPlane) = interp1(planeInfo(iPlane+1).frameTimes, zscore(neuralData(ex).allFcell(iPlane).FcellNeuAvg),periEventTimes,'previous');
                catch
                    alignedNeuropil{ev}(:,:,iPlane) = interp1(planeInfo(iPlane+1).frameTimes, zscore(neuralData(ex).allFcell(iPlane).FcellNeuAvg{1}),periEventTimes,'previous');
            end
    ...
    neuralData(ex).eta.alignedNeuropil = alignedNeuropil;
