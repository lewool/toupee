function figName = plotWhiskRasters(expInfo, behavioralData, neuralData,eyeData, whichCells,whichROIs, whichTrials, k)
...
%% plot neural activity (all trials)separated by whisking trials vs non-whisk trials
neuralfig = figure;
set(neuralfig, 'Position', [80 250 870 660]);
hold on;
...
while k <= max_k
    ...
    for a = 1:3
        title(strcat('cell',num2str(plotCells(k))))
        for iCond = 1:length(trialLists{a})
            [spidx1, spidx2] = goToRasterSubplot(length(trialLists), total_length, cellfun(@length, trialLists{a})', a, iCond, 1, psth);
            subplot(total_length,length(trialLists),[spidx1 spidx2])
    ...
    if was_a_key && strcmp(get(neuralfig, 'CurrentKey'), 'leftarrow')
      k = max(1, k - 1);
    elseif was_a_key && strcmp(get(neuralfig, 'CurrentKey'), 'rightarrow')
      k = min(max_k, k + 1);
    elseif was_a_key && strcmp(get(neuralfig, 'CurrentKey'), 'return')
        disp(k)
        %save = 'C:\Users\Ella Svahn\Documents\eyedata\LEW031';
...
while k <= max_k
    ...
    for a = 1:3
        for iCond = 1:length(trialLists{a})
            ...
            [spidx1, spidx2] = goToRasterSubplot(length(trialLists), total_length, cellfun(@length, trialLists{a})', a, iCond, 1, psth);
            ax = subplot(total_length,length(trialLists),[spidx1 spidx2]);
            f = imagesc(eventWindow,1:numTrials,alignedResps{a}(whichTrials(sortIdx),:,plotROIs(k)));
            colormap(ax,rasterColor(rasterColors(iCond,:)));
            hold on;
            %             line([0 0],[1 length(whichTrials)],'LineStyle','-','Color',[0 0 0]);
            mt = plot(relativeTimes(:,2,a),1:numTrials,'bo');
    ...
    elseif was_a_key && strcmp(get(facemapfig, 'CurrentKey'), 'return')
        disp(k)
        figName = strcat(expInfo.mouseName,'_',expInfo.expDate,'_ROI_',num2str(plotROIs(k)));
