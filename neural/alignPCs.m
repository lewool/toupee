function [neuralData] = alignPCs(expInfo, neuralData, behavioralData, timeBeforeAndAfter, events)
...
for ex = 1:length(expInfo)
    if ~isstruct(behavioralData(ex).eventTimes) || ~isstruct(behavioralData(ex).wheelMoves)
        ...
        for ev = 1:length(events)
            ...
            alignedPCs{ev} = zeros(nt, length(eventWindow), size(neuralData(ex).PCs,2));

            %grab cell responses associated with the event time windows 
            %(size is nTrials x windowLength x nCells)
            for iCell = 1:size(neuralData(ex).PCs,2)        
                alignedPCs{ev}(:,:,iCell) = interp1(neuralData(ex).respTimes,neuralData(ex).PCs(:,iCell),periEventTimes,'previous');
            end
