function [expInfo, eventTimes] = getEventTimes(expInfo, events, sessions)
...
%
% eventTimes : table
%   A table containing three types of event times: 'signalsTime',
%   'updateTime', and 'daqTime' (columns) for the specified sessions
%   (rows).
%
%
% Examples:
% ---------
% 1) For a single session, get the event times of....
%   deats = {'LEW031', '2020-02-03', 1};
%   files = {'block', 'timeline'};
%   expInfo = toupee.meta.processExperiment(deats, files);
%   ...
%
% 2) For multiple sessions, get the event times of...
%   deats = {{'LEW031', '2020-02-03', 1},...
...
% for events, use the exact fieldnames from the block file or you'll get an error
% e.g. events = {'stimulusOnTimes' 'interactiveOnTimes' 'stimulusOffTimes'};
...
if ~ischar(events) && (~iscell(events) || ~ischar(events{1}))
    error('toupee:behavioral:getEventTimes:badInput',...
          ['The "events" input arg must be a char array or cell array of ',...
           'chars.']);
end
% Get sessions info
if nargin < 3  % then use all sessions
...
for iE = 1:nE
    ...
    t = expInfo.TimelineFile{expRef};  % timeline data
...
% feedback signal generated.
    ...
    phdRaw = t.rawDAQData{1}(:, strcmp('photoDiode', t.hw.inputs{1}.name));
    ...
    [~, flipvalsFilt] = kmeans(phdFilt, 3); %[BLACK WHITE GRAY]
    [~, flipvalsRaw] = kmeans(phdRaw, 3); %[BLACK WHITE GRAY]
    ...
    for iName = 1:length(events)
        signalsUpdates{iName} = interp1(block.stimWindowUpdateTimes, block.stimWindowUpdateTimes, block.events.(events{iName}), 'nearest', 'extrap');
    end

    % set up the corrected-timestamp struct
    eventTimes = struct('event',events);
    ...
    for i = 1:length(signalsUpdates)
        ...
        eventTimes(i).signalsTime = getfield(block.events,events{i});
